...
<div class="subDiv2">
<ul>
@for(value of arr(); track $index){
<li>{{value}}</li>
}
</ul>
 <button [disabled]="sumArr() > 30" (click)="modifyArr()">Add Value</button>
...
export class EffectSignalComponent {
    constructor(){
      ...
      effect(() => {
        if(this.sumArr() > 30) alert(`Max alert reached ${this.sumArr()}`)
      })
    }
